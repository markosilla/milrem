...
class Server(asyncio.DatagramProtocol):
    ...
    def handle_ping(self):
        self.last_ping = time.time()
        self.transport.sendto(bytes('pong', encoding='utf8'), ('127.0.0.1', 12345))
    
    def datagram_received(self, data, addr):
        self.logger.debug("Got data: %s", str(data, encoding='utf8'))
        if len(data) == 4 and str(data, encoding='utf8') == 'ping':
            self.handle_ping()
        else:
    ...
    def send_packet(self, sensor_id, value_type, max_value, interval=1):
        ...
        if time.time() - self.last_ping > 5:
            self.logger.error("Ping timeout. Exiting ...")
            self.loop.stop()
            return
